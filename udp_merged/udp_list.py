...
if 2<= len(sys.argv) <= 3 and sys.argv[1] == 'server':
    ...
    while True:
        ...
        sent_hash = hashlib.md5(example.encode())
        ...
        while delta <= abort_after:
            ...
            try:
                ...
                received_hash = hashlib.md5(data.encode())
                print 'DEBUG Received Hash: ', (received_hash.hexdigest())
                print 'DEBUG Sent Hash: ',  (sent_hash.hexdigest())
                print 'Received size: ', len(data)
                print 'Sent size: ', len(example)
                print 'DEBUG len(data): ' , len(data)

                if(received_hash.hexdigest() == sent_hash.hexdigest()):
                    print 'yes'
                    b = time.time()
                    delta_packet = b-a #Time since sending for each packet
                    print delta_packet
                    print 'Throughput: ', (sys.getsizeof(example)/delta_packet), ' bytes/sec'
                else:
                    print 'no'
